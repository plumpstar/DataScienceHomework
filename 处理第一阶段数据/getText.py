...
file = 'D:\数据\文本\第一阶段\\1月1日.txt'
fp = open(file, 'a+', encoding='utf-8')

cnt = open('D:\数据\筛选\第一阶段\\1月1日.csv', 'r')
i = 1
for line in cnt:
    print(i)
    id = line.strip().split()[0]
    # tureUrl = boWenUrl + line
    # print(tureUrl)
    # res = requests.get(tureUrl, headers=headers)
    # dic = res.json()
    # fp.write(pq(dic.get('data').get('longTextContent')).text())
    # fp.write('\n')
